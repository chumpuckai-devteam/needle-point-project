-- V3: store follows + richer catalog seed + harden store write RLS
...
-- Harden stores writes: owner only (seed stores with null owner are service-role only)
...
-- Harden store_products writes: store owner only
...
-- Project-store links are public only for public projects; owners can see their
-- own private draft links. This avoids leaking private project ids.
...
-- Seed approved read-only catalog stores so fresh non-prod databases have shops
-- before products/tags are backfilled. Existing owner_user_id values are kept.
...
-- Seed catalog products (idempotent by name+store)
...
-- Link public projects to stores (Available at)
