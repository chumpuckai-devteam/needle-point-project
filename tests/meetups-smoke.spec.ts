...
test.describe("Stitching meetups smoke", () => {
  ...
  test("host check-in is wired in API and roster UI", async () => {
    ...
    expect(migrations).toContain("check_in_code");
    expect(migrations).toContain("confirm_meetup_seat");
    expect(migrations).toContain("hold_expires_at");
    expect(migrations).toContain("set_meetup_check_in_by_code");
    const api = readFileSync(resolve(root, "src/api/meetups.ts"), "utf8");
    expect(api).toContain("setMeetupGuestCheckInOnline");
    expect(api).toContain("confirmMeetupSeatOnline");
    expect(api).toContain("setMeetupCheckInByCodeOnline");
    ...
    expect(page).toContain("meetup-ticket-card");
    expect(page).toContain("Confirm seat");
    const analytics = readFileSync(resolve(root, "src/components/StoreOwnerAnalytics.tsx"), "utf8");
